...
primitive BytesToU16 
  ...
  // ((msb << 8) + lsb).u16()  Triggers a llvm bug request
    try 
      ...
      Debug("Couldn't read id data at " + __loc.file() + ":" +__loc.method_name() + " line " + __loc.line().string())
      0
    end 
...
primitive U8ToQos fun apply(value : U8) : (Qos | None) =>
  ...
  | 2 => Qos2
    ...
    Debug("Got invalid QoS value ("+value.string() + ") at" + __loc.file() + ":" +__loc.method_name() + " line " + __loc.line().string())
    None
  end 
...
primitive BytesToValue fun  apply(data : Array[U8] box) : USize =>
  ...
  if (index > 4) then Debug("Error") ; return 0 end
...
primitive RlByteCount fun  apply(data : Array[U8] box) : USize =>
  ...
  try encodedByte = data(startIndex + index)? end
  index = index + 1
  if (index > 4) then Debug("Remaining Length Overflow") ; return 0 end
  until ((encodedByte and 128) == 0) end
  index
